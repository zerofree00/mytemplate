        ...
        <van-nav-bar @click-left="onleft" @click-right="showSearch">
            <template #left>
                <div class="color_000">
                    ...
                    <span class="header_title">发文管理</span>
                </div>
            </template>
            <template #right>
                <van-icon
...
export default {
    ...
    data() {
        return {
            tablist: [
                { strName: '我的', strModeCode: '20001' },
                { strName: '部门', strModeCode: '20002' },
                { strName: '全部', strModeCode: '20003' },
            ],
            typeList: [
                { text: '全部', value: '' },
    ...
    activated() {
        if (localStorage.getItem('sendDel') == 'true') {
            this.intCurPage = 1
            this.list = []
            this.finished = true
            this.getpostList()
            this.isLoading = false
            localStorage.setItem('sendDel', 'false')
        }
    },
    methods: {
        hasAppData(e) {
        ...
        linkDetails(type, strWorkId, blShowDelete) {
            if (strWorkId.split('_')[1] == 'mainView') {
                this.$router.push({
                    path: '/post/detail',
                    query: { type, strWorkId, blShowDelete },
                })
            } else {
                this.$router.push({
                    path: '/post/post_edit',
