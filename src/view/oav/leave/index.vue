<template>
    <div class="getInfomation_index">
        <van-nav-bar @click-left="onleft" @click-right="showSearch">
            <template #left>
                <div class="color_000">
                    ...
                    <span class="header_title">请假管理</span>
                </div>
            </template>
            <template #right>
                <van-icon
                        ...
                        >
                            ...
                            <div class="time_item">
                                <van-icon
                                    ...
                                    name="shijian1"
                                />
                                <span>{{ item1.strStartTime }}</span
                                >~
                                <span>{{ item1.strEndTime }}</span>
                            </div>
                            <span
                                class="right_span"
                                :class="
        ...
        <!-- 新增申请 start-->
        <div class="add_apply" @click="addApply">
            <van-icon class="iconfont" class-prefix="icon" name="icon_add" />
        </div>
        <!-- 新增申请 end -->
    </div>
</template>
...
import { workflowCommonAdd } from '@/api/common'
...
export default {
    ...
    data() {
        return {
            tablist: [
                { strName: '我的请假', strModeCode: '37001' },
                { strName: '全部请假', strModeCode: '37002' },
            ],
            typeList: [
                { text: '全部', value: '' },
                { text: '请假事由', value: 'strVacateSeed' },
    ...
    activated() {
        if (
            localStorage.getItem('leaveDel') == 'true' ||
            localStorage.getItem('leaveAdd') == 'true'
        ) {
            this.intCurPage = 1
            this.list = []
            this.finished = true
            this.getleaveList()
            localStorage.setItem('leaveDel', 'false')
            localStorage.setItem('leaveAdd', 'false')
        }
    },
    methods: {
        hasAppData(e) {
        ...
        addApply() {
            ...
            workflowCommonAdd(param)
                .then((res) => {
                    if (res.data.intCode === 200) {
                        console.log(res.data.objData.objList[0])
                        this.$router.push({
                            path: '/leave/apply',
                            query: {
                                strWorkFlowId:
                                    res.data.objData.objList[0].strWorkFlowId,
                                strWorkId:
                                    res.data.objData.objList[0].strWorkId,
                            },
                        })
                    } else {
                        this.$toast(res.data.strMsg)
