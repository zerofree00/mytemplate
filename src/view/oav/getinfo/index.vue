<template>
    <div class="getInfomation_index">
        <van-nav-bar @click-left="onleft" @click-right="showSearch">
            <template #left>
                <div class="color_000">
                    ...
                    <span class="header_title">收文管理</span>
                </div>
            </template>
            <template #right>
                <van-icon
        ...
        <!-- 标签页 -->
        <van-tabs v-model="active" animated @change="changeTab" swipeable>
            <van-tab
...
<script>
import { receivedList } from '@/api/getInfo'
...
export default {
    ...
    data() {
        return {
            tablist: [
                { strName: '待签收文', strModeCode: '21001' },
                { strName: '我的收文', strModeCode: '21002' },
                { strName: '全部收文', strModeCode: '21003' },
            ],
            typeList: [
                { text: '全部', value: '' },
    ...
    activated() {
        if (localStorage.getItem('recDel') == 'true') {
            this.intCurPage = 1
            this.list = []
            this.finished = true
            this.getreceivedList()
            this.isLoading = false
            localStorage.setItem('recDel', 'false')
        }
    },
    methods: {
        hasAppData(e) {
        ...
        //去详情
        linkDetails(type, strWorkId, code, blShowDelete) {
            if (type == '21001') {
                if (code == '2') {
                    this.$router.push({
                        path: '/getinfo/detail_unsign',
                        query: { type: type, strWorkId: strWorkId },
                    })
                } else {
                    this.$router.push({
                        path: '/getinfo/detail',
                        query: { type: type, strWorkId: strWorkId, code: code },
                    })
                }
            } else {
